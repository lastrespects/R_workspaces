...
# 부서 별 평균 커미션과 최대 커미션 알아내기
# 1. filter
# 2. !is.na
# 3. group by + summarize
# 4. mean
# 5. max

result = emp %>% filter(!is.na(COMM))%>% group_by(DEPTNO)%>%
  summarize(AVG_COMM = mean(COMM), MAX_COMM = max(COMM))
View(result)